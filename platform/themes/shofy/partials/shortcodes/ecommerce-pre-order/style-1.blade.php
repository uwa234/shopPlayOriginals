    ...
    <div class="container">
        <div class="row align-items-center mb-40">
            <div class="col-xl-4 col-md-5 col-sm-6">
                ...
                @if($preOrder && $preOrder->expected_delivery_date)
                    <div class="pre-order-delivery-info mt-3">
                        <small class="text-muted">
                            <i class="ti ti-truck"></i>
                            {{ __('Expected delivery: :date', ['date' => $preOrder->expected_delivery_date->format('M d, Y')]) }}
                        </small>
                    </div>
                @endif
            </div>
                    ...
                    <div class="tp-product-offer-slider-active swiper-container">
                        <div class="swiper-wrapper">
                            @foreach($preOrder->products as $product)
                                @include(Theme::getThemeNamespace('views.ecommerce.includes.product-item'), ['class' => 'tp-product-offer-item swiper-slide mb-0', 'withCountdown' => true, 'endDate' => $preOrder->pre_order_end_date, 'isPreOrder' => true])
                            @endforeach
                        </div>
